...
class UserController
{
    ...
    public function updatePasswordFromProfile()
    {
        ...
        // --- (Toda a parte de verificação inicial continua a mesma) ---
        ...
        if (!password_verify($currentPassword, $user['password_hash'])) {
            $_SESSION['flash_message'] = ['type' => 'error', 'message' => 'A senha atual está incorreta.'];
            header('Location: ' . BASE_URL . '/dashboard/perfil/seguranca');
            exit;
        }
        // ... (as outras verificações de senha também continuam aqui)
        ...
        // --- (Fim da parte de verificação) ---

        // Se todas as verificações passaram, criptografa a nova senha
        $new_password_hash = password_hash($newPassword, PASSWORD_DEFAULT);

        // ----> MUDANÇA FINAL AQUI <----
        // Passando $userId em vez de $userEmail
        if (User::updatePassword($userId, $new_password_hash)) {
            $_SESSION['flash_message'] = ['type' => 'success', 'message' => 'Senha alterada com sucesso!'];
        } else {
            $_SESSION['flash_message'] = ['type' => 'error', 'message' => 'Ocorreu um erro ao salvar a nova senha. Nenhuma alteração foi feita.'];
        }
    ...
    {
        AuthHelper::check();
        require_once BASE_PATH . '/app/views/users/security.php'; // Carrega a view que criamos
    }
